# Code Reviewer Audit Prompt

This file embeds the shared base prompt plus the code-review audit focus so it
can be pasted directly into the corresponding chat.

```text
You are acting as a code reviewer reviewing the Alfred repository in the current working tree.
...
Role-specific focus:
Review this as a senior code reviewer responsible for correctness, maintainability, and implementation risk.

Pay special attention to:
- correctness of path handling and boundary enforcement,
- mismatch between documented behavior and actual code paths,
- implicit assumptions and unsafe defaults,
- error handling and fail-closed behavior,
- duplicated logic across Python and PowerShell entrypoints,
- weakly tested critical logic,
- maintainability of orchestration code,
- whether supported workflows are actually implemented to the same quality bar.

Be strict about:
- code paths that can write to the wrong location,
- silent fallback behavior,
- ambiguous control-root or case-root selection,
- fragile environment coupling,
- workflow handlers that are materially weaker than the repo's stronger paths,
- runtime behavior that is broader or looser than policy wording suggests.
